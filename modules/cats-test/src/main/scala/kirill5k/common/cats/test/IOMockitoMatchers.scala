...
trait IOMockitoMatchers extends MockitoMatchers {
  ...
  extension [A](stub: OngoingStubbing[fs2.Stream[IO, A]])
    def thenFailStream(error: Throwable): OngoingStubbing[fs2.Stream[IO, A]] = stub.thenReturn(fs2.Stream.raiseError[IO](error))
    def thenStream(items: A*): OngoingStubbing[fs2.Stream[IO, A]]            = stub.thenReturn(fs2.Stream.emits(items.toList))
    def thenStream(items: List[A]): OngoingStubbing[fs2.Stream[IO, A]]       = stub.thenReturn(fs2.Stream.emits(items))
    def thenReturnEmptyStream: OngoingStubbing[fs2.Stream[IO, A]]            = stub.thenReturn(fs2.Stream.empty)
